      ...
      embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0001):
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                conv_bn1 = tf.layers.batch_normalization(self.embedded_chars_expanded)
                ...
                conv_bn2 = tf.layers.batch_normalization(conv1)

                # Apply nonlinearity
                #h = tf.nn.relu(tf.nn.bias_add(conv_bn2, b), name="relu")
                h = tf.nn.leaky_relu(tf.nn.bias_add(conv_bn2, b),0.2 ,name="leakyRelu")
                #h = tf.nn.leaky_relu(tf.nn.bias_add(conv1, b),0.2 ,name="leakyRelu")
...
#                 self.confusion_matrix = tf.confusion_matrix(labels=tf.argmax(self.input_y, 1 ), predictions=self.predictions, num_classes=num_classes)    
