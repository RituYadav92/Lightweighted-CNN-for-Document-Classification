    ...
    with sess.as_default():
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            step, summaries, loss, accuracy = sess.run(
                ...
                feed_dict)
#             step, summaries, loss, accuracy,predictions_results, confusion_matrix = sess.run(
#                 [global_step, dev_summary_op, cnn.loss, cnn.accuracy, cnn.predictions, cnn.confusion_matrix],
#                 feed_dict)
            
            
#             np.savetxt(open("Predictions.csv", "w+"), predictions_results, fmt='%f')
#             np.savetxt(open("Ground_Truth_Labels.csv", "w+"), np.argmax(y_batch, 1), fmt='%f')
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, acc {:g}, cm{}".format(time_str, step, loss, accuracy))
#             print("{}: step {}, loss {:g}, acc {:g}, cm{}".format(time_str, step, loss, accuracy, confusion_matrix))
            if writer:
                writer.add_summary(summaries, step)
                

        print('Trainning input set: x_train, y_train',len(list(zip(x_train, y_train))))
#         print('Evaluation input set: x_dev, y_dev',len(list(zip(x_dev, y_dev))))
        # Generate batches
